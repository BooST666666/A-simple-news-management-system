	...
	<div class="con_right">
		<div>
			<div class="blog_list_wrap">
				<div class="con_header"><div class="title"><span>您当前的位置：删除新闻</span></div></div>
                ...
                <table class="table1">
					...
					if (!isset($_GET['page'])){
					   ...
					   if (is_numeric($_GET['page'])){                            //判断变量$page是否为数字，如果是则返回true
						...
						$query = "(select * from sport order by news_id desc)
                        union
                        (select * from science order by news_id desc)
                        union
                        (select * from music  order by news_id desc)
                        union
                        (select * from economics order by news_id desc)
                        union
                        (select * from cartoon order by news_id desc)";   
                        ...
                        union
                        select * from cartoon order by news_id desc limit $offset, $page_size");
					...
					<tr>
                        ...
                        <td align="center"><a href="delete_news_ok.php?id=<?php echo $row->news_id;?>&&catagory_id=<?php echo $row->catagory_id ?>"><img src="image/error-warning-fill.png" style="width: 30;height:30; border:0;"></a></td>
                      ...
                      <table style="width:100%; font-size:14px; border:0px;">
                        <tr>
                          <!--  翻页条 -->
							...
							echo  "<a href=delete_news.php?page=1 >首页</a>&nbsp;";        //显示“首页”超链接
							echo "<a href=delete_news.php?page=".($_GET['page']-1).">上一页</a>&nbsp;";      //显示“上一页”超链接
							}
							if($_GET['page']<$page_count){                                  //如果当前页不是尾页
							echo "<a href=delete_news.php?page=".($_GET['page']+1).">下一页</a>&nbsp;";       //显示“下一页”超链接
							echo "<a href=delete_news.php?page=".$page_count.">尾页</a>";                  //显示“尾页”超链接
							}
