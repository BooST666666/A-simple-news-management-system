...
$result = mysqli_query($link,"select * from science where news_id='{$newsid}'"); // 通过sql语言寻找表，找出指定信息
...
while ($rows=mysqli_fetch_array($result)){ //重点！！ 从结果集中取得一行作为关联数组 fetch data in array format 
	  ...
	  $rs=mysqli_query($link,"select * from review where state='已审核' and review_qu ='科技'and news_id='{$newsid}'");
...
而现今，动漫的发展已属于文化创意产业，同时是目前全世界热门且高人气的流行文化。</p>
		...
		<div class="review section">
			...
			<h5>评论</h5>
			<form name="form1" method="post" action="science_addreview.php">
